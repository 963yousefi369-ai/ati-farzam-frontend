...
import {
  Shield, Zap, Headphones, Wrench, Target, Phone, Mail, MapPin,
  Star, Heart, Award, Truck, Clock, CheckCircle, Users, Globe,
  Lock, Settings, BarChart3, Smartphone, Wifi, Battery, Signal,
  Navigation, Radio, Cpu, Satellite,
} from "lucide-react";
...
const ICON_OPTIONS = Object.keys(ICON_MAP).map((name) => ({ label: name, value: name }));
...
const SectionTitleBlock = ({ eyebrow, title, subtitle, align = "center" }: { eyebrow?: string; title: string; subtitle?: string; align?: "center" | "right" }) => (
  <div className={`mb-10 ${align === "center" ? "text-center" : ""}`}>
    {eyebrow && (
      <div className="inline-flex items-center gap-1.5 text-xs font-semibold tracking-widest mb-3 text-accent-600">
...
const FeatureCard = ({ icon, title, desc }: { icon?: string; title: string; desc: string }) => (
  <div className="rounded-2xl shadow-md bg-white p-6 md:p-8 text-center hover:shadow-xl transition-shadow">
    <div className="w-16 h-16 rounded-2xl bg-[#10b981]/10 flex items-center justify-center mx-auto mb-4">
      <SvgIcon name={icon} className="w-7 h-7 text-[#10b981]" />
    </div>
...
const ContactCardBlock = ({ icon, title, value, href }: { icon?: string; title: string; value: string; href?: string }) => (
  <div className="rounded-2xl shadow-md bg-white p-6 border border-gray-100 flex items-start gap-4">
    <div className="w-12 h-12 rounded-xl bg-[#f1f5f9] flex items-center justify-center shrink-0">
      <SvgIcon name={icon} className="w-5 h-5 text-[#1e3a5f]" />
    </div>
    <div>
...
const CtaBannerBlock = ({ title, subtitle, ctaText, ctaLink, bg = "dark" }: {
  ...
  >
    <div className="max-w-xl mx-auto px-4 text-center">
      ...
      {ctaText && (
        <a href={ctaLink || "#"} className="bg-primary-500 hover:bg-primary-400 text-white font-semibold px-6 py-3 rounded-xl transition inline-block">
          {ctaText}
        </a>
      )}
    </div>
    ...
    FeaturesGrid: {
      fields: {
        items: {
          ...
          defaultItemProps: { icon: "Star", title: "ویژگی", desc: "توضیحات" },
          arrayFields: {
            icon: { type: "select", label: "آیکون", options: ICON_OPTIONS },
    ...
    ContactCards: {
      fields: {
        cards: {
          ...
          label: "اطلاعات تماس",
          getItemSummary: (item: any) => item.title,
          defaultItemProps: { icon: "Phone", title: "تلفن", value: "021-12345678" },
          arrayFields: {
            icon: { type: "select", label: "آیکون", options: ICON_OPTIONS },
            title: { type: "text", label: "عنوان" },
  ...
  home: {
    content: [
      ...
      { type: "FeaturesGrid", props: { id: "why-1", items: [
        { icon: "Shield", title: "کیفیت ساخت بالا", desc: "استفاده از بهترین قطعات و تکنولوژی روز دنیا" },
        { icon: "Zap", title: "قیمت مناسب و رقابتی", desc: "قیمت‌های رقابتی بدون کاهش کیفیت" },
        { icon: "Headphones", title: "پشتیبانی در کنار شما", desc: "پشتیبانی فنی در کنار خرید و نصب ردیاب" },
      ] } },
  ...
  contact: {
    content: [
      ...
      { type: "ContactCards", props: { id: "cc-1", cards: [
        ...
        { icon: "MapPin", title: "آدرس دفتر", value: "تهران، ایران" },
      ] } },
      { type: "Spacer", props: { id: "sp-2", height: "80px" } },
