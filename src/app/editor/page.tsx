...
import { Plus, FileText, Edit3, Trash2, Loader2, Home, Info, Phone, Monitor } from "lucide-react";
import { motion } from "framer-motion";

const AVAILABLE_PAGES = [
  { slug: "home", title: "صفحه اصلی", icon: Home },
  { slug: "about", title: "درباره ما", icon: Info },
  { slug: "contact", title: "تماس با ما", icon: Phone },
  { slug: "software", title: "نرم‌افزار", icon: Monitor },
  { slug: "custom", title: "صفحه سفارشی", icon: FileText },
];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100" dir="rtl">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
            ...
            {AVAILABLE_PAGES.filter((p) => p.slug !== "custom").map((page) => {
              ...
              return (
                <motion.div
                  key={page.slug}
                  whileHover={{ y: -2 }}
                  className="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-lg hover:border-accent/30 transition-all"
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    {saved && (
                      <span className="text-[10px] bg-primary-50 text-accent-700 px-2 py-0.5 rounded-full font-medium">
                        ذخیره شده
                      </span>
                    )}
                  </div>
                  <Link
                    ...
                    className="w-full block text-center bg-primary hover:bg-primary-dark text-white text-sm font-medium py-2.5 rounded-xl transition-colors"
                  >
              ...
              {pages.map((page, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      href={`/editor/${page.slug}?title=${encodeURIComponent(page.title)}`}
                      className="bg-primary hover:bg-primary-dark text-white text-xs font-medium px-4 py-2 rounded-lg transition-colors"
                    >
                      ویرایش
                    </Link>
                    <Link
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    dir="ltr"
                    className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-mono focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20"
                  />
                  {newSlug && (
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="bg-accent hover:bg-accent-dark text-white text-sm font-medium px-6 py-2.5 rounded-xl transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    ایجاد و ویرایش
                  </button>
                  <button
        ...
        {/* Info */}
        <div className="mt-10 bg-primary-50 border border-teal-200 rounded-2xl p-6">
          <h3 className="font-bold text-accent-800 mb-2">راهنما</h3>
          <ul className="text-sm text-accent-700 space-y-1.5">
