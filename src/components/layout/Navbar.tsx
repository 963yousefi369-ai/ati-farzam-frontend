...
export default function Navbar() {
  ...
  useEffect(() => {
    const close = (event: KeyboardEvent) => event.key === 'Escape' && (setSearchOpen(false), setProductsOpen(false))
    ...
    <>
      <header className="sticky top-0 z-[var(--z-navbar)] border-b border-border-soft bg-white">
        <div className="mx-auto flex h-navbar max-w-7xl items-center gap-3 px-4 sm:px-6 lg:gap-5 lg:px-8">
          <Link href="/" aria-label={siteName || 'آتی فرزام ایرانیان'} className="shrink-0 rounded-xl">
            {logo ? (
              ...
              <span className="flex items-center gap-2.5">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary text-white" aria-hidden="true">
                  <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={1.9}><path d="M12 21s7-5.6 7-11a7 7 0 10-14 0c0 5.4 7 11 7 11z" /><circle cx="12" cy="10" r="2.4" /></svg>
                </span>
                  ...
                  {productsOpen && (
                    <div id="products-menu" className="absolute right-0 top-full z-[var(--z-dropdown)] w-[520px] pt-3">
                      <div className="grid grid-cols-2 gap-1 rounded-2xl border border-border-soft bg-white p-2 shadow-elevated">
                        {PRODUCT_CATEGORIES.map((category) => <Link key={category.href} href={category.href} className="min-h-20 rounded-xl p-3.5 transition-colors hover:bg-bg-soft"><span className="block text-sm font-semibold text-dark">{category.title}</span><span className="mt-1 block text-xs leading-5 text-text-muted">{category.desc}</span></Link>)}
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="mx-auto hidden w-full max-w-sm lg:block"><ProductSearch variant="bar" /></div>

          <div className="mr-auto flex items-center gap-2">
            <button type="button" aria-label="جستجو" onClick={() => setSearchOpen(true)} className="flex h-11 w-11 cursor-pointer items-center justify-center rounded-xl text-dark transition-colors hover:bg-bg-soft lg:hidden"><Search className="h-5 w-5" /></button>
            <Link href="/cart" aria-label={`سبد خرید، ${itemCount} کالا`} className="relative flex h-11 w-11 items-center justify-center rounded-xl text-dark transition-colors hover:bg-bg-soft"><ShoppingCart className="h-5 w-5" />{itemCount > 0 && <span className="absolute -left-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-accent px-1 text-[10px] font-semibold text-white">{itemCount.toLocaleString('fa-IR')}</span>}</Link>
            <AccountMenu />
            <button type="button" aria-label="بازکردن منو" aria-expanded={menuOpen} onClick={() => setMenuOpen(true)} className="flex h-11 w-11 cursor-pointer items-center justify-center rounded-xl text-dark transition-colors hover:bg-bg-soft xl:hidden"><Menu className="h-5 w-5" /></button>
          </div>
        </div>
      </header>

      {searchOpen && <div className="fixed inset-0 z-[var(--z-cmd-search-overlay)] bg-dark/50 lg:hidden" onClick={() => setSearchOpen(false)}><div className="bg-white p-4 shadow-elevated" onClick={(event) => event.stopPropagation()}><div className="mb-3 flex items-center justify-between"><span className="text-sm font-semibold text-dark">جستجو در محصولات</span><button type="button" aria-label="بستن جستجو" onClick={() => setSearchOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-xl hover:bg-bg-soft"><X className="h-5 w-5" /></button></div><ProductSearch variant="sheet" autoFocus onNavigate={() => setSearchOpen(false)} /></div></div>}

      <MobileMenu open={menuOpen} onClose={() => setMenuOpen(false)} onSearchClick={openMobileSearch} />
    </>
  )
}
