      ...
      {steps.map((step, idx) => {
        ...
        return (
          <motion.div
            ...
            transition={reduceMotion ? { duration: 0 } : { delay: idx * 0.08, duration: 0.3 }}
            className="flex gap-4"
          >
            <div className="flex flex-col items-center">
              <motion.div
                animate={!reduceMotion && step.stepStatus === 'current' ? { scale: [1, 1.1, 1] } : {}}
                transition={!reduceMotion && step.stepStatus === 'current' ? { duration: 2, repeat: Infinity } : { duration: 0 }}
                className={cn(
                  ...
                  step.stepStatus === 'current'   && 'bg-primary border-navy shadow-primary/20',
                  step.stepStatus === 'pending'   && 'bg-white border-border-soft',
                )}
              >
                {step.stepStatus === 'completed' && (
                  <Check className="w-4 h-4 text-white" />
                )}
                {step.stepStatus === 'current'   && <div className="w-3 h-3 rounded-full bg-white" />}
                {step.stepStatus === 'pending'   && <div className="w-1.5 h-1.5 rounded-full bg-text-muted/40" />}
              </motion.div>
              {!isLast && (
                <div className="relative w-0.5 flex-1 min-h-[2.5rem]">
                  ...
                  {step.stepStatus === 'completed' && (
                    <motion.div
                      ...
                      animate={reduceMotion ? {} : { scaleY: 1 }}
                      transition={reduceMotion ? { duration: 0 } : { duration: 0.3, delay: idx * 0.08 }}
            ...
            <div className={cn('pb-7 flex-1', isLast && 'pb-0')}>
              <p
                className={cn(
                  ...
                  step.stepStatus === 'current'   && 'text-primary',
                  step.stepStatus === 'pending'   && 'text-text-muted',
                )}
              >
                {step.label}
              </p>
              {step.date && (
                <p className="text-xs text-text-muted mt-1">{formatDate(step.date)}</p>
              )}
            </div>
          </motion.div>
        )
