...
export default function AddToCartButton({ product, quantity, imageUrl, inStock = true }: AddToCartButtonProps) {
  ...
  if (!shopEnabled) {
    return (
      <Button asChild className="w-full h-14 text-base rounded-xl bg-primary-600 hover:bg-primary-700 hover:shadow-lg gap-2.5 font-semibold" size="lg">
        <Link href="/contact">
  ...
  return (
    <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }}>
      <Button
        onClick={handleAdd}
        className="w-full h-14 text-base rounded-xl bg-primary hover:bg-primary-dark hover:shadow-lg gap-2.5 font-semibold relative overflow-hidden"
        size="lg"
      >
