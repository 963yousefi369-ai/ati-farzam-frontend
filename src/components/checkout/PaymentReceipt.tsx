...
import {
  CheckCircle2, XCircle, Printer, Phone, Clock, Info,
  MapPin, Package, Share2, ExternalLink, Copy, Home,
} from 'lucide-react'
...
export default function PaymentReceipt({ status, orderId, token }: PaymentReceiptProps) {
  ...
  // Move focus to result heading for screen reader announcement
  useEffect(() => {
    const timer = setTimeout(() => headingRef.current?.focus(), 350)
  ...
  if (isPaid) {
    return (
      <>
        ...
        <div className="flex flex-col items-center gap-6 py-6 receipt-print" aria-live="polite" role="status">
          {/* Success Icon */}
          <motion.div
          ...
          >
            <h1 ref={headingRef} tabIndex={-1} className="text-2xl font-semibold text-accent mb-1 outline-none">پرداخت موفق</h1>
          ...
          >
            <Card className="border-border-soft shadow-lg overflow-hidden">
              {/* Header */}
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-accent">پرداخت موفق</span>
                  </div>
                  <div className="text-left">
                ...
                {loading ? (
                  ...
                  /* Error fallback */
                  <div className="rounded-xl bg-warning-light border border-warning/20 p-4 space-y-2">
                  ...
                  <>
                      ...
                      <div className="space-y-2">
                        ...
                        {order.items.map((item: any, idx: number) => (
                          <div key={idx} className="flex items-center gap-3 py-1.5">
                            ...
                            <p className="text-primary font-semibold text-xs flex-shrink-0" dir="ltr">
                    ...
                    {/* Total */}
                    <div className="flex justify-between font-bold text-text-primary border-t border-border-soft pt-3 mt-1">
                      <span className="text-sm">جمع کل پرداخت‌شده</span>
                      <span className="text-lg text-accent" dir="ltr">
          ...
          <div className="flex flex-wrap gap-2 justify-center print:hidden">
            <Button asChild className="bg-primary hover:bg-primary-dark text-white gap-2 h-10 text-sm rounded-xl">
          ...
          <div className="flex items-center gap-2 print:hidden">
            ...
            <button
              ...
              className="w-11 h-11 rounded-lg bg-bg-secondary flex items-center justify-center hover:bg-bg-tertiary transition-colors active:scale-95"
              aria-label="کپی لینک"
            >
            ...
            <button
              ...
              className="w-11 h-11 rounded-lg bg-blue-50 flex items-center justify-center hover:bg-blue-100 transition-colors active:scale-95"
              aria-label="اشتراک در تلگرام"
            >
              <ExternalLink className="w-4 h-4 text-blue-500" aria-hidden="true" />
            </button>
            <button
              onClick={() => handleShare('whatsapp')}
              className="w-11 h-11 rounded-lg bg-green-50 flex items-center justify-center hover:bg-green-100 transition-colors active:scale-95"
  ...
  // ─── FAILED STATE ─────────────────────────────────────────────
  return (
    <div className="flex flex-col items-center gap-6 py-8" aria-live="polite" role="status">
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
      ...
      <motion.div
        ...
        className="text-center"
      >
        <h1 ref={headingRef} tabIndex={-1} className="text-2xl font-semibold text-error mb-1 outline-none">پرداخت ناموفق</h1>
        <p className="text-text-secondary text-sm mb-3">متأسفانه پرداخت انجام نشد</p>
        <div className="inline-flex items-center gap-2 bg-accent-light text-accent rounded-xl px-4 py-2 text-xs font-medium">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        <Button asChild className="bg-primary hover:bg-primary-dark text-white h-10 text-sm rounded-xl">
